...
class BitNetCppBridge:
    ...
    def generate(
        ...
        cmd = [
            ...
            str(top_k),
            "-t",
            str(os.cpu_count() or 4),  # Use all CPU threads
        ]
        ...
        try:
            ...
            stderr_thread = threading.Thread(
                target=drain_stderr, args=(process.stderr, stderr_output), daemon=True
            )
            stderr_thread.start()
            ...
            if process.stdout is not None:
                ...
                for byte_char in iter(
                    lambda: process.stdout.read(1) if process.stdout else b"", b""
                ):
                    ...
                    if char:  # Only add if decoder returned a character
                        ...
                        if not prompt_stripped and len(total_output) >= len(
                            full_prompt
                        ):
