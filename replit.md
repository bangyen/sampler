...
## Overview
This project is a stateless FastAPI web application with a JavaScript frontend that demonstrates different AI models across three tasks: zero-shot text classification, Named Entity Recognition (NER), and Optical Character Recognition (OCR). The application provides an interactive interface for comparing model performance without any persistence - results are displayed during the current session only. Models include efficient options like Microsoft's BitNet b1.58 2B, SmolLM2, Qwen2.5 for classification, BERT-based models for NER, and EasyOCR/PaddleOCR for text extraction.
...
### UI/UX Decisions
The application features a modern JavaScript frontend (HTML/CSS/JS) with a tabbed interface for Zero-Shot Classification, NER, and OCR. It boasts a fully responsive mobile design with a hamburger menu and off-canvas sidebar for an optimal user experience across devices. The design prioritizes clean layouts and smooth animations (300ms CSS transitions).

### Technical Implementations
The backend is built with FastAPI, providing REST APIs and Server-Sent Events (SSE) for real-time token streaming during zero-shot classification. Model loading is handled automatically from Hugging Face, with CPU-only inference for BitNet, SmolLM2, and Qwen2.5 models. The classification interface uses `TextIteratorStreamer` for token-by-token generation. Advanced generation parameters like temperature, top-p, top-k, and max tokens are configurable. Performance metrics (time, tokens, tokens/second) are tracked and displayed.
...
- **Zero-Shot Classification:** Interactive classification with optimized GGUF models (BitNet b1.58 2B GGUF - Fastest, SmolLM2 1.7B GGUF - Fast). Slower transformers models hidden by default. Features streaming responses and configurable generation parameters.
...
- **Tabbed Interface:** Separates Zero-Shot Classification, NER, and OCR functionalities into distinct tabs.
...
- **Stateless Design:** No persistence - all results are session-only for a simplified demo experience.
...
The system is designed for robustness, including graceful error handling and optional dependency loading for features like NER/OCR.
...
- **API Endpoint Tests** (`test_server.py`): Validates FastAPI endpoints using test client, covering model listing and NER/OCR configurations.
...
The test suite uses fixtures for consistent test data and mocking for external dependencies, ensuring fast, isolated tests that don't require actual models.
