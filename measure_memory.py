...
def load_qwen_05b():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        "Qwen/Qwen2.5-0.5B-Instruct", torch_dtype=torch.float16, low_cpu_mem_usage=True
    )
    model.to("cpu")
    return model
...
if __name__ == "__main__":
    ...
    try:
        results["SmolLM2 1.7B GGUF"] = measure_model(
            "SmolLM2 1.7B (GGUF)", load_smollm_gguf
        )
    except Exception:
        results["SmolLM2 1.7B FP16"] = measure_model(
            "SmolLM2 1.7B (FP16)", load_smollm_fp16
        )
    ...
    results["RoBERTa Large NER"] = measure_model(
        "RoBERTa Large NER", load_roberta_large_ner
    )
